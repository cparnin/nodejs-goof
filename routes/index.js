...
User.find({ username: validator.escape(String(req.body.username)), password: validator.escape(String(req.body.password)) }, function (err, users) {
      if (users.length > 0) {
...
Todo.findById(sanitize(req.params.id), function (err, todo) {
